...
describe('timezone — wall clock to UTC', () => {
  it('converts winter time in Jerusalem (UTC+2)', () => {
    ...
    expect(zonedWallTimeToUtc(2024, 1, 15, 9, 0, 0, JLM).toISOString()).toBe('2024-01-15T07:00:00.000Z');
  });

  it('converts summer time in Jerusalem (UTC+3, DST)', () => {
    // 2024-06-21 09:00 IDT = 06:00Z — the bug this replaces stored 09:00Z
    expect(zonedWallTimeToUtc(2024, 6, 21, 9, 0, 0, JLM).toISOString()).toBe('2024-06-21T06:00:00.000Z');
  });

  it('handles the day before and after the spring-forward transition', () => {
    // Israel DST 2024 began 2024-03-29 02:00 -> 03:00
    expect(zonedWallTimeToUtc(2024, 3, 28, 12, 0, 0, JLM).toISOString()).toBe('2024-03-28T10:00:00.000Z');
    expect(zonedWallTimeToUtc(2024, 3, 30, 12, 0, 0, JLM).toISOString()).toBe('2024-03-30T09:00:00.000Z');
  });

  it('handles the day before and after the fall-back transition', () => {
    // Israel DST 2024 ended 2024-10-27 02:00 -> 01:00
    expect(zonedWallTimeToUtc(2024, 10, 26, 12, 0, 0, JLM).toISOString()).toBe('2024-10-26T09:00:00.000Z');
    expect(zonedWallTimeToUtc(2024, 10, 28, 12, 0, 0, JLM).toISOString()).toBe('2024-10-28T10:00:00.000Z');
  });

  it('supports half-hour offset zones', () => {
    // Asia/Kolkata is UTC+5:30 year round
    expect(zonedWallTimeToUtc(2024, 6, 21, 9, 0, 0, 'Asia/Kolkata').toISOString()).toBe('2024-06-21T03:30:00.000Z');
  });
  ...
  it('is the inverse of utcToZonedParts', () => {
    ...
    expect(zonedWallTimeToUtc(p.year, p.month, p.day, p.hour, p.minute, p.second, JLM).toISOString()).toBe(
      instant.toISOString(),
    );
  });
...
describe('timezone — parsing and validation', () => {
  it('parses date plus time strings', () => {
    expect(zonedDateTimeToUtc('2024-06-21', '09:00', JLM).toISOString()).toBe('2024-06-21T06:00:00.000Z');
  });

  it('treats a missing time as local midnight', () => {
    expect(zonedDateTimeToUtc('2024-06-21', undefined, JLM).toISOString()).toBe('2024-06-20T21:00:00.000Z');
  });
